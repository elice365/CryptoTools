...
import { rabbitEncrypt, rabbitDecrypt } from "@/lib/crypto/symmetric";
import { CryptoError, cryptoUtils } from "@/lib/utils";

export function RabbitTools() {
  ...
  const handleGenerateKey = () => {
    const keyBytes = crypto.getRandomValues(new Uint8Array(16)); // Rabbit uses 16-byte keys
  ...
  const handleEncrypt = async () => {
    ...
    try {
      const result = await rabbitEncrypt(input, {
        key,
  ...
  const handleDecrypt = async () => {
    ...
    setIsProcessing(true);
    try {
      const plaintext = await rabbitDecrypt(input, {
        key,
        keyEncoding: outputEncoding,
        outputEncoding,
      });
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>{t("rabbit.title")}</CardTitle>
          <CardDescription>{t("rabbit.description")}</CardDescription>
        </CardHeader>

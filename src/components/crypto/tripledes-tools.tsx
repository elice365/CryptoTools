...
import { tripleDesEncrypt, tripleDesDecrypt } from "@/lib/crypto/symmetric";
import { CryptoError, cryptoUtils } from "@/lib/utils";

export function TripleDesTools() {
  ...
  const handleGenerateKey = () => {
    const keyBytes = crypto.getRandomValues(new Uint8Array(24)); // 3DES uses 24-byte keys
  ...
  const handleGenerateIV = () => {
    const ivBytes = crypto.getRandomValues(new Uint8Array(8)); // 3DES uses 8-byte IV
    const generated = outputEncoding === "hex"
  ...
  const handleEncrypt = async () => {
    ...
    try {
      const result = await tripleDesEncrypt(input, {
        key,
  ...
  const handleDecrypt = async () => {
    ...
    setIsProcessing(true);
    try {
      const plaintext = await tripleDesDecrypt(input, {
        key,
        keyEncoding: outputEncoding,
        iv: mode === "ecb" ? undefined : iv,
        ivEncoding: outputEncoding,
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>{t("tripledes.title")}</CardTitle>
          <CardDescription>{t("tripledes.description")}</CardDescription>
        </CardHeader>
